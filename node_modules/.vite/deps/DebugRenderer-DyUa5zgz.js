...
import { B as Bounds, P as BigPool, nt as Rectangle } from "./Geometry-DdrlfpC8.js";
import "./lib-d7LMjOCR.js";
import { l as Container } from "./Filter-DPsohYYD.js";
...
//#endregion
//#region node_modules/@pixi/layout/dist/core/debug/calculateDebugRegions.mjs
...
function calculateRegions(layout, regions) {
	...
	const marginRegion = regions.get("margin");
	...
	const borderRegion = regions.get("border");
	...
	const paddingRegion = regions.get("padding");
...
function calculateFlexRegion(layout, regions) {
	var _a;
	const flexRegion = regions.get("flex");
	const paddingRegion = regions.get("padding");
	flexRegion.outer.copyFrom(paddingRegion.inner);
	const bounds = new Bounds();
	const children = layout.yoga.getChildCount();
	for (let i = 0; i < children; i++) {
		const computedBounds = layout.yoga.getChild(i).getComputedLayout();
		bounds.addRect(new Rectangle(computedBounds.left, computedBounds.top, computedBounds.width, computedBounds.height));
	}
	...
	(_a = flexRegion.inner) == null || _a.copyFrom(bounds.rectangle);
	regions.get("content").outer.copyFrom(bounds.rectangle);
}
//#endregion
//#region node_modules/@pixi/layout/dist/core/debug/DebugNode.mjs
...
var DebugNode = class extends Container {
	...
	*/
	init(regions) {
		...
		if (!regions.heatOnly) Object.entries(regions).forEach(([type, region]) => {
			...
			if (region.inner) this.drawCutBox(graphics, region.outer, region.inner, region.color, alpha);
			else {
		...
		if (invalidationCount > 0 && draw) {
			const normalizedAlpha = Math.min(invalidationCount / 20, 1);
			...
			this.heatGraphics.rect(marginRegion.outer.x, marginRegion.outer.y, marginRegion.outer.width, marginRegion.outer.height);
			this.heatGraphics.fill({
				color,
				alpha: Math.min(.3, normalizedAlpha)
			});
			this.heatGraphics.stroke({
				color,
				alpha: Math.max(.3, normalizedAlpha),
				pixelLine: true
			});
		}
...
//#endregion
//#region node_modules/@pixi/layout/dist/core/debug/DebugRenderer.mjs
var DebugRenderer_exports = /* @__PURE__ */ __exportAll({ DebugRenderer: () => DebugRenderer });
...
var DebugRenderer = class {
	...
	*/
	render(layout) {
		...
		const regionData = Object.values(DebugRegionType).reduce((acc, type) => {
...
//#endregion
export { BoxSizing as a, Edge as c, Justify as d, Overflow as f, Align as i, FlexDirection as l, Wrap as m, DebugRenderer_exports as n, Direction as o, PositionType as p, loadYoga as r, Display as s, DebugRenderer as t, Gutter as u };

//# sourceMappingURL=DebugRenderer-DyUa5zgz.js.map