  ...
  static _handleCommand(line) {
    ...
    switch (type) {
      ...
      case osuClasses.CommandType.VectorScale: {
        ...
        this._timelineGroup?.vectorScale.add(type, easing, startTime, endTime, new osuClasses.Vector2(startX, startY), new osuClasses.Vector2(endX, endY));
      ...
      case osuClasses.CommandType.Movement: {
        ...
        this._timelineGroup?.x
          .add(osuClasses.CommandType.MovementX, easing, startTime, endTime, startX, endX);

        this._timelineGroup?.y
          .add(osuClasses.CommandType.MovementY, easing, startTime, endTime, startY, endY);
      ...
      case osuClasses.CommandType.Color: {
        ...
        this._timelineGroup?.color.add(type, easing, startTime, endTime, new osuClasses.Color4(startRed, startGreen, startBlue, 1), new osuClasses.Color4(endRed, endGreen, endBlue, 1));
...
class SlidableObject extends osuClasses.HitObject {
  ...
  repeats = 0;
  velocity = 1;
  path = new osuClasses.SliderPath();
    ...
    if (frames) {
      ...
      frames.forEach((frame) => {
        ...
        const encodedData = [
          ...
          legacyFrame?.buttonState ?? osuClasses.ReplayButtonState.None,
        ];
...
class BeatmapEncoder {
  ...
  async encodeToPath(path, beatmap) {
    ...
    try {
      ...
      await promises.writeFile(path, await this.encodeToString(beatmap));
    }
    catch (err) {
...
class StoryboardEncoder {
  async encodeToPath(path, storyboard) {
    ...
    try {
      await promises.mkdir(node_path.dirname(path), { recursive: true });
      await promises.writeFile(path, await this.encodeToString(storyboard));
    }
    catch (err) {
      const reason = err.message || err;
...
class ScoreEncoder {
  ...
  async encodeToPath(path, score, beatmap) {
    ...
    try {
      await promises.mkdir(node_path.dirname(path), { recursive: true });
      await promises.writeFile(path, new Uint8Array(data));
    }
    catch (err) {
      const reason = err.message || err;
  ...
  async encodeToBuffer(score, beatmap) {
    ...
    try {
      ...
      writer.writeInteger(score.replay?.gameVersion ?? ScoreEncoder.DEFAULT_GAME_VERSION);
